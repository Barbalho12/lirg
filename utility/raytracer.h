...
class RayTracer{
	...
	public:
		...
		void init(Reader reader_){
			...
			transformation = Transformation();
			// transformation.scalation(scene.getObject(0), 0.5);
			// transformation.translation(scene.getObject(0), 0, -0.2, 0);
			// transformation.translation(scene.getObject(scene.getObjects().size()-3), 1, 0, 0);
			// transformation.scalation(scene.getObject(0), 2, 2, 2);
			// transformation.rotation(scene.getObject(0), *scene.getObject(0)->getPoints()[0], 0.0, 45.0, 0.0);
