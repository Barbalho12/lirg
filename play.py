...
for n in range(0, frames):
    ...
    text += "NAME =  samples/video/img_"+str(n)+".ppm\n"
    text += "TYPE = PPM\n"
    ...
    text += "SHADER = TOON\n"
    text += "\n"
    text += "SPHERE = 0 0 -1 0.5 TOON 0.0 0.3 0.8 ; 0 0 0 ; \n"
    text += "COLORS { \n"
    text += "1.0 0.0 0.0\n"
    text += "0.9 0.0 0.0\n"
    text += "0.8 0.0 0.0\n"
    ...
    text += "LIGHT = POINT ; 1 1 1 ; 8 "+str(x)+" 5\n"
    text += "NATURAL_LIGHT = 0.4 0.4 0.4\n"
    text += "\n"
    text += "LOWER_LEFT_CORNER = -2 -1 -1\n"
    text += "H_D_VIEW_PLANE = 4 0 0 \n"
    text += "V_D_VIEW_PLANE = 0 2 0 \n"
    text += "CAMERA_ORIGIN = 0 0 0\n"
    text += "\n"
    ...
    os.system("convert samples/video/img_"+str(n)+".ppm samples/video/img_"+str(n)+".jpg")
    images += "samples/video/img_"+str(n)+".jpg "
    
os.system("rm samples/video/*.txt && rm samples/video/*.ppm")    
os.system("convert -delay 20 -loop 0 "+images+" video.gif")
